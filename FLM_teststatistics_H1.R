...
phi <- sin(2*pi*t^3)^3           #regression function
...
run_experiment <- function(seed) {
  ...
  snr_data <- numeric(n)
  for (i in 1:n) {
    snr_data[i] <- trapezoidal_integral(X[,i] * phi, dt)^2
  }
  var <- mean(snr_data) * ((1/snr) - 1)
  sigma <- sqrt(var)
...
for(j in 1:m) {
  ...
  T_n_hyp[j] <- ifelse(abs(T_n_list[j]) < gaussian_quantile, "accept", "reject")
...
power_D_n <- mean(D_n_list > chi_squared_quantile)
power_T_n <- mean(T_n_list > gaussian_quantile)

cat("empirical Power of D_n under", m, "Observations:", power_D_n, "\n")
cat("empirical Power of T_n under", m, "Observations:", power_T_n, "\n")
variance <- var(D_n_list > chi_squared_quantile)
cat("snr:", snr, "\n")
